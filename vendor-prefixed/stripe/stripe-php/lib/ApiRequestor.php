...
class ApiRequestor
{
    ...
    private static function _encodeObjects($d)
    {
        if ($d instanceof ApiResource) {
            return \GiftFlow\Vendor\Stripe\Util\Util::utf8($d->id);
        }
        if (true === $d) {
            return 'true';
        }
        ...
        return \GiftFlow\Vendor\Stripe\Util\Util::utf8($d);
    }
    /**
    ...
    private static function _specificV1APIError($rbody, $rcode, $rheaders, $resp, $errorData)
    {
        ...
        switch ($rcode) {
            case 400:
                ...
                if ('rate_limit' === $code) {
                    return \GiftFlow\Vendor\Stripe\Exception\RateLimitException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code, $param);
                }
            ...
            // no break
            case 404:
                return \GiftFlow\Vendor\Stripe\Exception\InvalidRequestException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code, $param);
            case 401:
                return \GiftFlow\Vendor\Stripe\Exception\AuthenticationException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code);
            case 402:
                return \GiftFlow\Vendor\Stripe\Exception\CardException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code, $declineCode, $param);
            case 403:
                return \GiftFlow\Vendor\Stripe\Exception\PermissionException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code);
            case 429:
                return \GiftFlow\Vendor\Stripe\Exception\RateLimitException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code, $param);
            default:
                return \GiftFlow\Vendor\Stripe\Exception\UnknownApiErrorException::factory($msg, $rcode, $rbody, $resp, $rheaders, $code);
        }
    }
    /**
     * @static
    ...
    private static function _specificOAuthError($rbody, $rcode, $rheaders, $resp, $errorCode)
    {
        ...
        switch ($errorCode) {
            case 'invalid_client':
                return \GiftFlow\Vendor\Stripe\Exception\OAuth\InvalidClientException::factory($description, $rcode, $rbody, $resp, $rheaders, $errorCode);
            case 'invalid_grant':
                return \GiftFlow\Vendor\Stripe\Exception\OAuth\InvalidGrantException::factory($description, $rcode, $rbody, $resp, $rheaders, $errorCode);
            case 'invalid_request':
                return \GiftFlow\Vendor\Stripe\Exception\OAuth\InvalidRequestException::factory($description, $rcode, $rbody, $resp, $rheaders, $errorCode);
            case 'invalid_scope':
                return \GiftFlow\Vendor\Stripe\Exception\OAuth\InvalidScopeException::factory($description, $rcode, $rbody, $resp, $rheaders, $errorCode);
            case 'unsupported_grant_type':
                return \GiftFlow\Vendor\Stripe\Exception\OAuth\UnsupportedGrantTypeException::factory($description, $rcode, $rbody, $resp, $rheaders, $errorCode);
            case 'unsupported_response_type':
                return \GiftFlow\Vendor\Stripe\Exception\OAuth\UnsupportedResponseTypeException::factory($description, $rcode, $rbody, $resp, $rheaders, $errorCode);
            default:
                return \GiftFlow\Vendor\Stripe\Exception\OAuth\UnknownOAuthErrorException::factory($description, $rcode, $rbody, $resp, $rheaders, $errorCode);
        }
    }
    /**
     * @static
    ...
    private function _requestRaw($method, $url, $params, $headers, $apiMode, $usage, $maxNetworkRetries)
    {
        ...
        $requestStartMs = \GiftFlow\Vendor\Stripe\Util\Util::currentTimeMillis();
        ...
        if (isset($rheaders['request-id']) && \is_string($rheaders['request-id']) && '' !== $rheaders['request-id']) {
            self::$requestTelemetry = new RequestTelemetry($rheaders['request-id'], \GiftFlow\Vendor\Stripe\Util\Util::currentTimeMillis() - $requestStartMs, $usage);
        }
    ...
    private function _requestRawStreaming($method, $url, $params, $headers, $apiMode, $usage, $readBodyChunkCallable, $maxNetworkRetries)
    {
        ...
        if (isset($rheaders['request-id']) && \is_string($rheaders['request-id']) && '' !== $rheaders['request-id']) {
            self::$requestTelemetry = new RequestTelemetry($rheaders['request-id'], \GiftFlow\Vendor\Stripe\Util\Util::currentTimeMillis() - $requestStartMs);
        }
        return [$rbody, $rcode, $rheaders, $myApiKey];
    }
    /**
    ...
    {
        if (!self::$_httpClient) {
            self::$_httpClient = \GiftFlow\Vendor\Stripe\HttpClient\CurlClient::instance();
        }
    ...
    public static function streamingHttpClient()
    {
        if (!self::$_streamingHttpClient) {
            self::$_streamingHttpClient = \GiftFlow\Vendor\Stripe\HttpClient\CurlClient::instance();
        }
