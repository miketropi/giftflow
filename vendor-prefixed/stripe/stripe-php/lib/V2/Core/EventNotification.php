...
use GiftFlow\Vendor\Stripe\Reason;
use GiftFlow\Vendor\Stripe\RelatedObject;
use GiftFlow\Vendor\Stripe\Util\EventNotificationTypes;
 ...
 * @property null|\GiftFlow\Vendor\Stripe\StripeContext        $context  Authentication context needed to fetch the event or related object.
...
abstract class EventNotification
{
     ...
     * @param \GiftFlow\Vendor\Stripe\StripeClient $client a StripeClient instance that this can use to make requests
     */
    public function __construct($json, $client)
    {
        ...
        if (\array_key_exists('context', $json) && null !== $json['context']) {
            $this->context = \GiftFlow\Vendor\Stripe\StripeContext::parse($json['context']);
        }
     ...
     * should use \GiftFlow\Vendor\Stripe\BaseStripeClient#parseEventNotification instead for
    ...
    protected function fetchRelatedObject()
    {
        ...
        return $this->client->deserialize($response->body, \GiftFlow\Vendor\Stripe\Util\Util::getApiMode($this->related_object->url));
    }
}
