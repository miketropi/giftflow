...
trait Request
{
     ...
     *
     * @throws \GiftFlow\Vendor\Stripe\Exception\InvalidArgumentException if $params exists and is not an array
     */
    protected static function _validateParams($params = null)
    {
        if ($params && !\is_array($params)) {
            $message = 'You must pass an array as the first argument to Stripe API '
                ...
                . "would be: \"GiftFlow\\Vendor\\Stripe\\Charge::create(['amount' => 100, "
                . "'currency' => 'usd', 'source' => 'tok_1234'])\")";

            throw new \GiftFlow\Vendor\Stripe\Exception\InvalidArgumentException($message);
        }
    }
    ...
    /**
     * @param string $url URL for the request
     * @param class-string< \Stripe\Collection|\GiftFlow\Vendor\Stripe\SearchResult > $resultClass indicating what type of paginated result is returned
     * @param null|array $params list of parameters for the request
     * @param null|array|string $options
     * @param string[] $usage names of tracked behaviors associated with this request
     *
     * @return \GiftFlow\Vendor\Stripe\Collection|\GiftFlow\Vendor\Stripe\SearchResult
     *
     * @throws \GiftFlow\Vendor\Stripe\Exception\ApiErrorException if the request fails
     */
    protected static function _requestPage($url, $resultClass, $params = null, $options = null, $usage = [])
    {
        ...
        if (!$obj instanceof $resultClass) {
            throw new \GiftFlow\Vendor\Stripe\Exception\UnexpectedValueException(
    ...
    {
        $opts = \GiftFlow\Vendor\Stripe\Util\RequestOptions::parse($options);
        $baseUrl = isset($opts->apiBase) ? $opts->apiBase : static::baseUrl();
        $requestor = new \GiftFlow\Vendor\Stripe\ApiRequestor($opts->apiKey, $baseUrl);
