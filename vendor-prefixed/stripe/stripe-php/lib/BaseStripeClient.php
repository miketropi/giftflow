...
use GiftFlow\Vendor\Stripe\Util\Util;
...
class BaseStripeClient implements StripeClientInterface, StripeStreamingClientInterface
{
    ...
    const DEFAULT_CONFIG = [
        ...
        'stripe_account' => null,
        'stripe_context' => null,
        'stripe_version' => \GiftFlow\Vendor\Stripe\Util\ApiVersion::CURRENT,
    ...
    private $config;
    /** @var \GiftFlow\Vendor\Stripe\Util\RequestOptions */
    private $defaultOpts;
    /**
     ...
     * - stripe_context (null|string|\GiftFlow\Vendor\Stripe\StripeContext): a Stripe account or compartment ID. If set, all requests sent by the client
    ...
    public function __construct($config = [])
    {
        ...
        $this->config = $config;
        $this->defaultOpts = \GiftFlow\Vendor\Stripe\Util\RequestOptions::parse(['stripe_account' => $config['stripe_account'], 'stripe_context' => $config['stripe_context'], 'stripe_version' => $config['stripe_version'], 'max_network_retries' => $config['max_network_retries']]);
    }
    /**
    ...
    public function requestCollection($method, $path, $params, $opts)
    {
        ...
        } else if (!$obj instanceof \GiftFlow\Vendor\Stripe\V2\Collection) {
            $received_class = \get_class($obj);
