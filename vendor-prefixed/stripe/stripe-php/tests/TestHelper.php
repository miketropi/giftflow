...
trait TestHelper
{
    ...
    protected function setUpConfig()
    {
        ...
        $this->clientMock = $this->createMock('\GiftFlow\Vendor\Stripe\HttpClient\ClientInterface');
        $this->streamingClientMock = $this->createMock('\GiftFlow\Vendor\Stripe\HttpClient\StreamingClientInterface');
        // By default, use the real HTTP client
        ApiRequestor::setHttpClient(\GiftFlow\Vendor\Stripe\HttpClient\CurlClient::instance());
        ApiRequestor::setStreamingHttpClient(\GiftFlow\Vendor\Stripe\HttpClient\CurlClient::instance());
    }
    /** @after */
    ...
    protected function expectsRequest($method, $path, $params = null, $headers = null, $hasFile = false, $base = null)
    {
        ...
        $this->prepareRequestMock($method, $path, $params, $headers, $hasFile, $base)->willReturnCallback(static function ($method, $absUrl, $headers, $params, $hasFile) {
            $curlClient = \GiftFlow\Vendor\Stripe\HttpClient\CurlClient::instance();
     ...
     * @param null|string $base base URL (e.g. 'https://api.stripe.com')
     */
    protected function expectsRequestStream($method, $path, $params = null, $headers = null, $hasFile = false, $base = null)
    {
        ApiRequestor::setStreamingHttpClient($this->streamingClientMock);
        $this->prepareRequestStreamMock($method, $path, $params, $headers, $hasFile, $base)->willReturnCallback(static function ($method, $absUrl, $readBodyChunkCallable, $headers, $params, $hasFile) {
            $curlClient = \GiftFlow\Vendor\Stripe\HttpClient\CurlClient::instance();
    ...
    {
        $this->prepareRequestMock($method, $path, $params, $headers, $hasFile, $base)->willReturn([\json_encode($response), $rcode, []]);
    }
    /**
    ...
    private function prepareRequestMock($method, $path, $params = null, $headers = null, $hasFile = false, $base = null)
    {
        ...
        $absUrl = $base . $path;
        return $this->clientMock->expects(static::once())->method('request')->with(
            ...
            null === $headers ? static::anything() : static::callback(\is_callable($headers) ? $headers : static function ($array) use ($headers) {
     ...
     * @return \PHPUnit\Framework\MockObject\Builder\InvocationMocker
     */
    private function prepareRequestStreamMock($method, $path, $params = null, $headers = null, $hasFile = false, $base = null)
    {
        ApiRequestor::setStreamingHttpClient($this->streamingClientMock);
        if (null === $base) {
            $base = Stripe::$apiBase;
        ...
        $absUrl = $base . $path;
        return $this->streamingClientMock->expects(static::once())->method('requestStream')->with(
            static::identicalTo(\strtolower($method)),
            static::identicalTo($absUrl),
            // for headers, we only check that all of the headers provided in $headers are
            // present in the list of headers of the actual request
