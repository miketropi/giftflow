...
namespace GiftFlow\Vendor\Stripe\HttpClient;

/**
 * @internal
 *
 * @covers \GiftFlow\Vendor\Stripe\HttpClient\CurlClient
 */
final class CurlClientTest extends \GiftFlow\Vendor\Stripe\TestCase
{
    use \GiftFlow\Vendor\Stripe\TestHelper;
    use \GiftFlow\Vendor\Stripe\TestServer;
    ...
    {
        $this->origMaxNetworkRetries = \GiftFlow\Vendor\Stripe\Stripe::getMaxNetworkRetries();
        $this->origMaxNetworkRetryDelay = \GiftFlow\Vendor\Stripe\Stripe::getMaxNetworkRetryDelay();
        $this->origInitialNetworkRetryDelay = \GiftFlow\Vendor\Stripe\Stripe::getInitialNetworkRetryDelay();
    }
    /**
     * @before
     */
    ...
    {
        $stripeReflector = new \ReflectionClass('\GiftFlow\Vendor\Stripe\Stripe');
        ...
        $curlClientReflector = new \ReflectionClass('\GiftFlow\Vendor\Stripe\HttpClient\CurlClient');
    ...
    {
        \GiftFlow\Vendor\Stripe\Stripe::setMaxNetworkRetries($this->origMaxNetworkRetries);
    ...
    {
        $fakeRandomGenerator = $this->createMock('\GiftFlow\Vendor\Stripe\Util\RandomGenerator');
    ...
    public function testDefaultOptions()
    {
        ...
        $this->expectException('GiftFlow\Vendor\Stripe\Exception\UnexpectedValueException');
    ...
    public function testShouldRetryOnTimeout()
    {
        ...
        \GiftFlow\Vendor\Stripe\Stripe::setMaxNetworkRetries(2);
    ...
    public function testShouldNotRetryAtMaximumCount()
    {
        $curlClient = new CurlClient();
        // call works when `maxNetworkRetries` is provided directly
        self::assertFalse($this->shouldRetryMethod->invoke($curlClient, 0, 0, [], \GiftFlow\Vendor\Stripe\Stripe::getMaxNetworkRetries(), null));
        // and when the arg is `null` and the value is read globally instead
        \GiftFlow\Vendor\Stripe\Stripe::setMaxNetworkRetries(2);
        self::assertFalse($this->shouldRetryMethod->invoke($curlClient, 0, 0, [], \GiftFlow\Vendor\Stripe\Stripe::getMaxNetworkRetries(), null));
    }
    ...
    public function testSleepTimeShouldGrowExponentially()
    {
        ...
        self::assertSame(\GiftFlow\Vendor\Stripe\Stripe::getInitialNetworkRetryDelay() * 1, $this->sleepTimeMethod->invoke($curlClient, 1, []));
        self::assertSame(\GiftFlow\Vendor\Stripe\Stripe::getInitialNetworkRetryDelay() * 2, $this->sleepTimeMethod->invoke($curlClient, 2, []));
        self::assertSame(\GiftFlow\Vendor\Stripe\Stripe::getInitialNetworkRetryDelay() * 4, $this->sleepTimeMethod->invoke($curlClient, 3, []));
        self::assertSame(\GiftFlow\Vendor\Stripe\Stripe::getInitialNetworkRetryDelay() * 8, $this->sleepTimeMethod->invoke($curlClient, 4, []));
    }
    ...
    public function testSleepTimeShouldAddSomeRandomness()
    {
        ...
        $baseValue = \GiftFlow\Vendor\Stripe\Stripe::getInitialNetworkRetryDelay() * (0.5 * (1 + $randomValue));
        // the initial value cannot be smaller than the base,
        // so the randomness is ignored
        self::assertSame(\GiftFlow\Vendor\Stripe\Stripe::getInitialNetworkRetryDelay(), $this->sleepTimeMethod->invoke($curlClient, 1, []));
    ...
    {
        \GiftFlow\Vendor\Stripe\Stripe::setMaxNetworkRetries(0);
        $curlClient = new CurlClient();
    ...
    {
        \GiftFlow\Vendor\Stripe\Stripe::setMaxNetworkRetries(3);
        $curlClient = new CurlClient();
        $curlOpts = $this->constructCurlOptionsMethod->invoke($curlClient, 'post', '', [], '', [], 'v2');
        $headers = $curlOpts[\CURLOPT_HTTPHEADER];
        self::assertTrue($this->hasHeader($headers, 'Idempotency-Key'));
    ...
    {
        $charge = \GiftFlow\Vendor\Stripe\Charge::all();
    ...
    public function testSetRequestStatusCallback()
    {
        try {
            ...
            \GiftFlow\Vendor\Stripe\ApiRequestor::setHttpClient($curl);
            \GiftFlow\Vendor\Stripe\Charge::all();
            self::assertTrue($called);
        } finally {
            \GiftFlow\Vendor\Stripe\ApiRequestor::setHttpClient(null);
        }
    }
    /**
     * @after
    ...
    public function testExecuteStreamingRequestWithRetriesHandlesDisconnect()
    {
        ...
        self::assertSame('GiftFlow\Vendor\Stripe\Exception\ApiConnectionException', \get_class($exception));
    ...
    {
        $curl = new CurlClient();
        $coupon = \GiftFlow\Vendor\Stripe\Coupon::retrieve('coupon_xyz');
        $absUrl = \GiftFlow\Vendor\Stripe\Stripe::$apiBase . '/v1/coupons/xyz';
        $opts[\CURLOPT_HTTPGET] = 1;
        $opts[\CURLOPT_URL] = $absUrl;
        $opts[\CURLOPT_HTTPHEADER] = ['Authorization: Basic c2tfdGVzdF94eXo6'];
