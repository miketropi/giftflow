 ...
 * @internal
 *
 * @covers \GiftFlow\Vendor\Stripe\Issuing\Dispute
 */
final class DisputeTest extends \GiftFlow\Vendor\Stripe\TestCase
{
    ...
    {
        $this->expectsRequest('post', '/v1/issuing/disputes/' . self::TEST_RESOURCE_ID, []);
    ...
    {
        $resource = Dispute::retrieve(self::TEST_RESOURCE_ID);
        $this->expectsRequest('post', '/v1/issuing/disputes/' . self::TEST_RESOURCE_ID . '/submit');
        $resource->submit();
        self::assertInstanceOf(Dispute::class, $resource);
    }
}