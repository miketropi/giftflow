 ...
 * @internal
 *
 * @covers \GiftFlow\Vendor\Stripe\Util\RequestOptions
 */
final class RequestOptionsTest extends \GiftFlow\Vendor\Stripe\TestCase
{
    ...
    {
        $opts = RequestOptions::parse(['api_key' => 'foo', 'idempotency_key' => 'foo']);
        self::assertSame('foo', $opts->apiKey);
        self::assertSame(['Idempotency-Key' => 'foo'], $opts->headers);
        self::assertNull($opts->apiBase);
    }
    ...
    {
        $opts = RequestOptions::parse(['api_key' => 'foo', 'foo' => 'bar']);
        self::assertSame('foo', $opts->apiKey);
        self::assertSame([], $opts->headers);
        self::assertNull($opts->apiBase);
    }
    public function testParseArrayWithAPIKeyAndUnexpectedKeysStrict()
    {
        ...
        $opts = RequestOptions::parse(['api_key' => 'foo', 'foo' => 'bar'], true);
    }
    public function testParseArrayWithAPIBase()
    {
        $opts = RequestOptions::parse(['api_base' => 'https://example.com']);
        self::assertNull($opts->apiKey);
        self::assertSame([], $opts->headers);
    ...
    {
        $baseOpts = RequestOptions::parse(['api_key' => 'foo', 'idempotency_key' => 'foo']);
        $opts = $baseOpts->merge(['idempotency_key' => 'bar']);
        self::assertSame('foo', $opts->apiKey);
    ...
    {
        $opts = RequestOptions::parse(['stripe_account' => 'foo', 'stripe_context' => 'foo', 'idempotency_key' => 'foo']);
