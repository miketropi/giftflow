...
use GiftFlow\Vendor\Stripe\Exception\TemporarySessionExpiredException;
use GiftFlow\Vendor\Stripe\HttpClient\CurlClient;
/**
 * @internal
 *
 * @covers \GiftFlow\Vendor\Stripe\ApiRequestor
 */
final class ApiRequestorTest extends TestCase
{
    ...
    public function testEncodeObjects()
    {
        ...
        $v = ['customer' => "\xe9"];
        $enc = $method->invoke(null, $v);
        self::assertSame($enc, ['customer' => "é"]);
        // Encodes booleans
        $v = true;
        $enc = $method->invoke(null, $v);
    ...
    public function testDefaultHeaders()
    {
        ...
        self::assertSame($headers['User-Agent'], 'Stripe/v1 PhpBindings/' . Stripe::VERSION . ' MyTestApp/1.2.34 (https://mytestapp.example)');
        self::assertSame($headers['Stripe-Version'], Stripe::getApiVersion());
    ...
    {
        $this->stubRequest('POST', '/v1/charges', [], null, false, ['error' => ['type' => 'invalid_request_error', 'message' => 'Missing id', 'param' => 'id']], 400);
        try {
            Charge::create();
            self::fail('Did not raise error');
        } catch (\GiftFlow\Vendor\Stripe\Exception\InvalidRequestException $e) {
    ...
    {
        $this->stubRequest('POST', '/v1/charges', [], null, false, ['error' => ['type' => 'idempotency_error', 'message' => "Keys for idempotent requests can only be used with the same parameters they were first used with. Try using a key other than 'abc' if you meant to execute a different request."]], 400);
        try {
            Charge::create();
            self::fail('Did not raise error');
        } catch (\GiftFlow\Vendor\Stripe\Exception\IdempotencyException $e) {
            self::assertSame(400, $e->getHttpStatus());
            self::compatAssertIsArray($e->getJsonBody());
    ...
    {
        $this->stubRequest('POST', '/v1/charges', [], null, false, ['error' => ['type' => 'invalid_request_error', 'message' => 'You did not provide an API key.']], 401);
        try {
            Charge::create();
            self::fail('Did not raise error');
        } catch (\GiftFlow\Vendor\Stripe\Exception\AuthenticationException $e) {
    ...
    {
        $this->stubRequest('POST', '/v1/charges', [], null, false, ['error' => ['type' => 'card_error', 'message' => 'Your card was declined.', 'code' => 'card_declined', 'decline_code' => 'generic_decline', 'charge' => 'ch_declined_charge', 'param' => 'exp_month']], 402);
        try {
            Charge::create();
            self::fail('Did not raise error');
        } catch (\GiftFlow\Vendor\Stripe\Exception\CardException $e) {
    ...
    {
        $this->stubRequest('GET', '/v1/accounts/foo', [], null, false, ['error' => ['type' => 'invalid_request_error', 'message' => "The provided key 'sk_test_********************1234' does not have access to account 'foo' (or that account does not exist). Application access may have been revoked."]], 403);
        try {
            Account::retrieve('foo');
            self::fail('Did not raise error');
        } catch (\GiftFlow\Vendor\Stripe\Exception\PermissionException $e) {
    ...
    {
        $this->stubRequest('GET', '/v1/charges/foo', [], null, false, ['error' => ['type' => 'invalid_request_error', 'message' => 'No such charge: foo', 'param' => 'id']], 404);
        try {
            Charge::retrieve('foo');
            self::fail('Did not raise error');
        } catch (\GiftFlow\Vendor\Stripe\Exception\InvalidRequestException $e) {
    ...
    {
        $this->stubRequest('POST', '/v1/charges', [], null, false, ['error' => ['message' => 'Too many requests']], 429);
        try {
            Charge::create();
            self::fail('Did not raise error');
        } catch (\GiftFlow\Vendor\Stripe\Exception\RateLimitException $e) {
    ...
    {
        $this->stubRequest('POST', '/v1/charges', [], null, false, ['error' => ['code' => 'rate_limit', 'message' => 'Too many requests']], 400);
        try {
            Charge::create();
            self::fail('Did not raise error');
        } catch (\GiftFlow\Vendor\Stripe\Exception\RateLimitException $e) {
    ...
    {
        $this->stubRequest('POST', '/oauth/token', [], null, false, ['error' => 'invalid_request', 'error_description' => 'No grant type specified'], 400, Stripe::$connectBase);
        try {
            OAuth::token();
            self::fail('Did not raise error');
        } catch (\GiftFlow\Vendor\Stripe\Exception\OAuth\InvalidRequestException $e) {
            self::assertSame(400, $e->getHttpStatus());
    ...
    {
        $this->stubRequest('POST', '/oauth/token', [], null, false, ['error' => 'invalid_client', 'error_description' => 'No authentication was provided. Send your secret API key using the Authorization header, or as a client_secret POST parameter.'], 401, Stripe::$connectBase);
        try {
            OAuth::token();
            self::fail('Did not raise error');
        } catch (\GiftFlow\Vendor\Stripe\Exception\OAuth\InvalidClientException $e) {
            self::assertSame(401, $e->getHttpStatus());
    ...
    {
        $this->stubRequest('POST', '/oauth/token', [], null, false, ['error' => 'invalid_grant', 'error_description' => 'This authorization code has already been used. All tokens issued with this code have been revoked.'], 400, Stripe::$connectBase);
        try {
            OAuth::token();
            self::fail('Did not raise error');
        } catch (\GiftFlow\Vendor\Stripe\Exception\OAuth\InvalidGrantException $e) {
            self::assertSame(400, $e->getHttpStatus());
    ...
    {
        $this->stubRequest('POST', '/oauth/token', [], null, false, ['error' => 'invalid_scope', 'error_description' => 'Invalid scope provided: invalid_scope.'], 400, Stripe::$connectBase);
        try {
            OAuth::token();
            self::fail('Did not raise error');
        } catch (\GiftFlow\Vendor\Stripe\Exception\OAuth\InvalidScopeException $e) {
            self::assertSame(400, $e->getHttpStatus());
    ...
    {
        $this->stubRequest('POST', '/oauth/token', [], null, false, ['error' => 'unsupported_grant_type'], 400, Stripe::$connectBase);
        try {
            OAuth::token();
            self::fail('Did not raise error');
        } catch (\GiftFlow\Vendor\Stripe\Exception\OAuth\UnsupportedGrantTypeException $e) {
            self::assertSame(400, $e->getHttpStatus());
    ...
    {
        $this->stubRequest('POST', '/oauth/token', [], null, false, ['error' => 'unsupported_response_type', 'error_description' => "Only 'code' response_type is supported, but 'unsupported_response_type' was provided"], 400, Stripe::$connectBase);
        try {
            OAuth::token();
            self::fail('Did not raise error');
        } catch (\GiftFlow\Vendor\Stripe\Exception\OAuth\UnsupportedResponseTypeException $e) {
            self::assertSame(400, $e->getHttpStatus());
    ...
    {
        $this->stubRequest('GET', '/v2/core/events/evt_123', [], null, false, ['error' => ['type' => 'temporary_session_expired', 'code' => 'session_bad', 'message' => 'you messed up']], 400, BaseStripeClient::DEFAULT_API_BASE);
        try {
    ...
    {
        $this->stubRequest('GET', '/v2/core/events/evt_123', [], null, false, ['error' => ['code' => 'invalid_request', 'message' => 'your request is invalid', 'param' => 'invalid_param']], 400, BaseStripeClient::DEFAULT_API_BASE);
        try {
            $client = new StripeClient('sk_test_123');
            $client->v2->core->events->retrieve('evt_123');
            self::fail('Did not raise error');
    ...
    public function testHeaderStripeVersionGlobal()
    {
        ...
        $this->stubRequest('POST', '/v1/charges', [], ['Stripe-Version: 2222-22-22'], false, ['id' => 'ch_123', 'object' => 'charge']);
        Charge::create();
    }
    ...
    public function testHeaderStripeAccountGlobal()
    {
        ...
        $this->stubRequest('POST', '/v1/charges', [], ['Stripe-Account: acct_123'], false, ['id' => 'ch_123', 'object' => 'charge']);
        Charge::create();
    }
    ...
    {
        $this->stubRequest('POST', '/v1/charges', [], static function ($array) {
            ...
            return true;
        }, false, ['id' => 'ch_123', 'object' => 'charge']);
        Charge::create([], ['stripe_account' => null]);
    }
    public function testHeaderStripeContextRequestOptions()
    {
        $this->stubRequest('POST', '/v2/billing/meter_event_session', [], ['Stripe-Context: wksp_123'], false, ['object' => 'billing.meter_event_session'], 200, BaseStripeClient::DEFAULT_API_BASE);
        $client = new StripeClient('sk_test_123');
    ...
    {
        $this->expectException(\GiftFlow\Vendor\Stripe\Exception\InvalidRequestException::class);
        $this->compatExpectExceptionMessageMatches('#null byte#');
        Charge::retrieve("abc_123\x00");
    }
    public function testRaisesForNullBytesInRawRequest()
    {
        $this->expectException(\GiftFlow\Vendor\Stripe\Exception\InvalidRequestException::class);
        $this->compatExpectExceptionMessageMatches('#null byte#');
        $client = new BaseStripeClient(['api_key' => 'sk_test_client']);
        $client->rawRequest('get', "/v1/xyz\x00");
    }
}