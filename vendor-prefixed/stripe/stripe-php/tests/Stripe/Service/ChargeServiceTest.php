 ...
 * @internal
 *
 * @covers \GiftFlow\Vendor\Stripe\Service\ChargeService
 */
final class ChargeServiceTest extends \GiftFlow\Vendor\Stripe\TestCase
{
    ...
    public function testAll()
    {
        ...
        self::assertInstanceOf(\GiftFlow\Vendor\Stripe\Charge::class, $resources->data[0]);
    }
    ...
    {
        $this->expectsRequest('post', '/v1/charges/' . self::TEST_RESOURCE_ID . '/capture');
        $resource = $this->service->capture(self::TEST_RESOURCE_ID);
        self::assertInstanceOf(\GiftFlow\Vendor\Stripe\Charge::class, $resource);
    }
    public function testCreate()
    {
        $this->expectsRequest('post', '/v1/charges');
        $resource = $this->service->create(['amount' => 100, 'currency' => 'usd', 'source' => 'tok_123']);
        self::assertInstanceOf(\GiftFlow\Vendor\Stripe\Charge::class, $resource);
    }
    public function testRetrieve()
    {
        $this->expectsRequest('get', '/v1/charges/' . self::TEST_RESOURCE_ID);
        $resource = $this->service->retrieve(self::TEST_RESOURCE_ID);
        self::assertInstanceOf(\GiftFlow\Vendor\Stripe\Charge::class, $resource);
    }
    public function testUpdate()
    {
        $this->expectsRequest('post', '/v1/charges/' . self::TEST_RESOURCE_ID);
