...
$app = new Slim\App();
$client = new \GiftFlow\Vendor\Stripe\StripeClient($api_key);
$app->post('/webhook', static function ($request, $response) use ($client, $webhook_secret) {
    ...
    try {
        ...
        if ($event_notification instanceof \GiftFlow\Vendor\Stripe\Events\V1BillingMeterErrorReportTriggeredEventNotification) {
        ...
        } elseif ($event_notification instanceof \GiftFlow\Vendor\Stripe\Events\UnknownEventNotification) {
