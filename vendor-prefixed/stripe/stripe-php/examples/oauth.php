...
require '../init.php';
\GiftFlow\Vendor\Stripe\Stripe::setApiKey(\getenv('STRIPE_SECRET_KEY'));
\GiftFlow\Vendor\Stripe\Stripe::setClientId(\getenv('STRIPE_CLIENT_ID'));
if (isset($_GET['code'])) {
    // The user was redirected back from the OAuth form with an authorization code.
    $code = $_GET['code'];
    try {
        $resp = \GiftFlow\Vendor\Stripe\OAuth::token(['grant_type' => 'authorization_code', 'code' => $code]);
    } catch (\GiftFlow\Vendor\Stripe\Exception\OAuth\OAuthErrorException $e) {
    ...
    try {
        \GiftFlow\Vendor\Stripe\OAuth::deauthorize(['stripe_user_id' => $accountId]);
    } catch (\GiftFlow\Vendor\Stripe\Exception\OAuth\OAuthErrorException $e) {
        exit('Error: ' . $e->getMessage());
    }
    ...
    $url = \GiftFlow\Vendor\Stripe\OAuth::authorizeUrl(['scope' => 'read_only']);
